...
QFrame *Ui::MainWindow::getCell(const Pos &position) const
{
    ...
    qWarning("Frame '%s' not found as an item in layout '%s'. Layout item count: %d", qPrintable(object_name),
             qPrintable(horizon_name), horizon->count());
    return nullptr;
}
...
void Ui::MainWindow::setCellColor(const Pos &position, const std::optional<QString> &color)
{
    ...
    throw std::runtime_error(
        QString("Cell frame '%1' not found").arg(QString::number(getFrameId(position))).toStdString());
}
