...
void Ui::MainWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    case MAIN_MENU: // 主菜单状态
        if (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) // 回车键开始游戏
        {
            context.game = Game();      
    ...
    case PAUSE: // 暂停状态
        if (event->key() == Qt::Key_P || event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) // 'P'键或回车键取消暂停
        {
            context.status = PLAYING;   
            timer.start(GAME_TIMER_INTERVAL_MS); 
            syncBoardAndActionToUi();   
    ...
    case GAME_OVER: // 游戏结束状态
        if (event->key() == Qt::Key_Return || event->key() == Qt::Key_Enter) // 回车键返回主菜单
        {
            context = Context();        
